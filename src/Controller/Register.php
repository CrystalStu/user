...
class Register extends AbstractController {
    ...
    public function verifyRegistrationAction(Request $request, UserPasswordEncoderInterface $passwordEncoder, \Swift_Mailer $mailer) {
        ...
        if (!$rawUserid) $user = $this->get('security.token_storage')
                                      ->getToken()
                                      ->getUser(); else $user = $this->getDoctrine()
        ...
        if (time() - $user->getLastSent() > 180 && (!$form->isSubmitted() && $form->isValid())) {
            ...
            $em->persist($user);
            $em->flush();
            $message = (new \Swift_Message())->setFrom(array('master@mail.cstu.gq' => 'Crystal User System'))
